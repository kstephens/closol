...
#|
 ;; for PLT:
 (define (matrix-image-plt m)
   (matrix-make-graymap! m)
   (let ((d (matrix-data-vector m)))
     (set! d (map (lambda (v) (make-color v v v)) (vector->list d)))
     (color-list->image d (matrix-width m) (matrix-height m) 0 0)
    ))
|#

 ;; for PLT:
 (define (matrix-image-ppm m filename)
   (matrix-make-graymap! m)
   (let* ((v (matrix-data-vector m))
	  (bv (make-bytevector (* (vector-length v) 3)))
	  (bvi 0)
	  )
     (vector-for-each 
       (lambda (v) 
	 (bytevector-u8-set! bv bvi v)
	 (set! bvi (+ bvi 1))
	 (bytevector-u8-set! bv bvi v)
	 (set! bvi (+ bvi 1))
	 (bytevector-u8-set! bv bvi v)
     ...
     (let ((port (open-file-output-port filename)))
       (display "P6\n" port)
       (display (matrix-width)) (display "\n" port)
       (display (matrix-height)) (display "\n" port)
       (display "255\n")
...
#|
(define m (matrix 10 10 ))
(matrix-fxy! m 
             (lambda (x y) 
               ; (display (list x y))(newline)
               (* x y))
             -5 5 -5 5)
(display (matrix-min-max m)) (newline)
(display (matrix-data-vector m)) (newline)
|#
